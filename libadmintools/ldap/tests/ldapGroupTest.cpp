...
void ldapGroupTest::testCn() {
  y::ldap::group  & group = y::ldap::Server().getGroup(y::ldap::CN("6INF1"), false);
...
void ldapGroupTest::testEditable() {
  ...
  y::ldap::group  & group2 = y::ldap::Server().getGroup(y::ldap::CN("directie"), true);
  if (!group2.editable()) {
    CPPUNIT_ASSERT(false);
  }
}
...
void ldapGroupTest::testFlagForCommit() {
  ...
  y::ldap::Server().clear();
  y::ldap::group  & group2 = y::ldap::Server().getGroup(y::ldap::CN("6INF1"), false);
...
void ldapGroupTest::testMembers() {
  ...
  {
    y::ldap::Server().clear();
    y::ldap::group  & group = y::ldap::Server().getGroup(y::ldap::CN("directie"), true);
    container<std::string> & members = group.members();
  ...
  // reload list
  {
    ...
    for(int i = 0; i < members.elms(); i++)  {
      if(members[i].compare("yvan@sanctamaria-aarschot.be") == 0) {
        members.remove(i);
        break;
      }
    }
...
void ldapGroupTest::testOwners() {
  {
    y::ldap::Server().clear();
    y::ldap::group  & group = y::ldap::Server().getGroup(y::ldap::CN("6INF1"), false);
    container<std::string> & owners = group.owners();
  ...
  // reload list
  {
    ...
    for(int i = 0; i < owners.elms(); i++)  {
      if(owners[i].compare("yvan@sanctamaria-aarschot.be") == 0) {
        owners.remove(i);
        break;
      }
    }
...
void ldapGroupTest::testSave() {
  ...
  {
    y::ldap::Server().clear();
    y::ldap::group & group = y::ldap::Server().getGroup(y::ldap::CN("newgroup"), false);
    if(!group.isNew()) {
  ...
  {
    y::ldap::Server().clear();
    y::ldap::group & group = y::ldap::Server().getGroup(y::ldap::CN("newgroup"), true);
    if(!group.isNew()) {
      // this should be a new group
      CPPUNIT_ASSERT(false);
    }
